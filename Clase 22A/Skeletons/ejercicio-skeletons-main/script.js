function showLoadingSkeleton(name, order) {
  ...
  pokemonCards.forEach(pokeCard => {
    pokeCard.classList.add('oculto')
  })

  const main = document.querySelector("main");
  const cardSkeletonContainer = document.createElement("div");
  cardSkeletonContainer.className = "card-loader";

  cardSkeletonContainer.innerHTML = `
    <img
      class="imagen-loader"
      id="imagen-pokemon-${order}"
    />
    <div class="card-body cuerpo-card-loader">
      <h5 class="card-title titulo-card-loader" id="nombre-pokemon-${order}"></h5>
      <p class="card-text texto-card-loader"></p>
      <span class="nombre-skill-loader"></span>
      <div class="contenedor-barra-loader" style="width:100%">
      </div>
      <span class="nombre-skill-loader"></span>
      <div class="contenedor-barra-loader" style="width:100%">
      </div>
  ...
  main.appendChild(cardSkeletonContainer);
...
async function fillPokemonData(name, order) {
  ...
  let pokemonCards = document.querySelectorAll('.card')
  pokemonCards.forEach(pokeCard => {
    pokeCard.classList.remove('oculto')
  })
  let skeletonCards = document.querySelectorAll('.card-loader')
  skeletonCards.forEach(pokeSkeletonCard => {
    pokeSkeletonCard.classList.add('oculto')
  })
